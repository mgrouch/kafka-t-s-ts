...
import com.example.txcache.app.domain.*; import com.example.txcache.core.*; import com.fasterxml.jackson.core.type.TypeReference; import com.fasterxml.jackson.databind.ObjectMapper;
import org.apache.kafka.common.serialization.Serdes; import org.apache.kafka.streams.*; import org.apache.kafka.streams.processor.*; import org.apache.kafka.streams.state.*;
import java.util.*;
@SuppressWarnings("deprecation")
public final class StreamsApp{
  static final String STORE="product-state-store";
  private final AppProperties props; private final ObjectMapper mapper; private final BatchProcessor<TEntity,SEntity,TSEntity> processor;
  public StreamsApp(AppProperties props,ObjectMapper mapper,BatchProcessor<TEntity,SEntity,TSEntity> processor){this.props=props;this.mapper=mapper;this.processor=processor;}
  public KafkaStreams buildAndStart(){
    ...
    streams.start(); return streams;
  }
  static final class ProductProcessor implements Processor<String,InputEvent<TEntity,SEntity,TSEntity>>{
    private final BatchProcessor<TEntity,SEntity,TSEntity> processor; private ProcessorContext context; private KeyValueStore<String,ProductState<TEntity,SEntity,TSEntity>> store;
    ProductProcessor(BatchProcessor<TEntity,SEntity,TSEntity> processor){this.processor=processor;}
    public void init(ProcessorContext context){this.context=context;this.store=context.getStateStore(STORE);}
